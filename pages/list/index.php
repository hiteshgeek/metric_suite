...
/**
 * Saved Lists Page
 * Displays list of all saved list widget configurations
 */
...
<head>
    ...
    <title>Saved Lists - Metric Suite</title>
...
<body>
    ...
    <main class="ms-saved-items" data-base-path="<?= htmlspecialchars($basePath) ?>">
        <header class="ms-saved-items__header">
            <div class="ms-saved-items__title-section">
                <h1 class="ms-saved-items__title">Saved Lists</h1>
                <p class="ms-saved-items__subtitle">Manage your list widgets</p>
            </div>
            <div class="ms-saved-items__actions">
                <a href="<?= $basePath ?>/pages/list/demo.php" class="ms-btn ms-btn--outline">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <polygon points="5 3 19 12 5 21 5 3"/>
                    </svg>
                    View Demo
                </a>
                <a href="<?= $basePath ?>/pages/list/configurator.php" class="ms-btn ms-btn--primary">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="ms-saved-graphs__toolbar">
            <div class="ms-saved-graphs__search">
                ...
                <input type="text" class="ms-input" id="ms-search-input" placeholder="Search lists...">
            </div>
        </div>
    ...
    <div class="ms-modal" id="ms-delete-modal" style="display: none;">
        ...
        <div class="ms-modal__content">
            <div class="ms-modal__header">
                <h3 class="ms-modal__title">Delete List</h3>
                <button class="ms-modal__close" id="ms-delete-modal-close">&times;</button>
            </div>
            <p class="ms-modal__message">Are you sure you want to delete this list? This action cannot be undone.</p>
    ...
    <script type="module">
        ...
        // List type labels
        const typeLabels = {
            'list-simple': 'Simple List',
            'list-ranked': 'Ranked List',
            'list-grouped': 'Grouped List',
            ...
            if (data.length === 0) {
                grid.innerHTML = `
                    <div class="ms-saved-graphs__empty" style="grid-column: 1 / -1;">
                        ...
                        <h3>No Lists Yet</h3>
                        <p>Create your first list widget to get started.</p>
                        <a href="${basePath}/pages/list/configurator.php" class="ms-btn ms-btn--primary">Create List</a>
                    </div>
                `;
                return;
            }
            ...
            grid.innerHTML = data.map(item => `
                <div class="ms-item-card" data-id="${item.id}">
                    ...
                    <div class="ms-item-card__actions">
                        <a href="${basePath}/pages/list/configurator.php?id=${item.id}" class="ms-btn ms-btn--outline ms-btn--sm">Edit</a>
        ...
        document.getElementById('ms-delete-confirm')?.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`${basePath}/api/list.php?id=${itemToDelete}`, {
                    method: 'DELETE'
                });
                const result = await response.json();
